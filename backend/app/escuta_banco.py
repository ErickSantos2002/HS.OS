...
⚠️ **O evento leva `{tabela, op, id}` e nada mais, de propósito.** Dois motivos,
e o segundo é o que manda:
...
2. **Segurança.** Um tópico de tabela é assinado por todo mundo que observa
   ...
   RLS decide. O custo é uma ida a mais para as poucas telas que usavam
   `payload.new`; o benefício é não ter uma segunda cópia da autorização para
   manter em dia.
...
from app.realtime import hub, topico_tabela
...
def _ao_receber(_conexao, _pid, _canal, carga: str) -> None:
    ...
    try:
        ...
        hub.publicar(
            topico_tabela(evento["tabela"]),
            "mudanca",
            {"tabela": evento["tabela"], "op": evento["op"], "id": evento.get("id")},
        )
