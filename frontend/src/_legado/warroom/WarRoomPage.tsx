...
interface Agente {
  ...
  contexto: number; filhos: { nome: string; vivo: boolean }[]; parceiro: string | null;
...
interface Feed {
  ...
  agentes: Agente[];
  eventos: Evento[];
  numeros: { entregas: number; conversas: number; tokens: number; custo: number; cacheTaxa: number };
  /** Consultas que falharam — aparecem discretas no rodapé em vez de sumir. */
  avisos?: string[];
  layout?: { nos?: Record<string, { x: number; y: number }>; zoom?: number; panX?: number; panY?: number };
}
...
export default function WarRoomPage() {
  ...
  useEffect(() => {
    let vivo = true;
    // Mesma porta de todas as telas do app: o invoke já carrega a sessão de
    // quem está logado. (A chave na URL segue aceita pela função, para o dia
    // em que a TV rodar sozinha, sem computador espelhando.)
    const puxar = async () => {
      const { data, error } = await supabase.functions.invoke("warroom-feed", {
        body: chave ? { k: chave } : {},
      });
      if (!vivo) return;
      if (error) {
        setErro("Não consegui carregar o painel. Confira se a função warroom-feed foi publicada.");
        return;
      }
  ...
  useEffect(() => {
    if (sujo || !feed?.layout) return;
    setPosManual(feed.layout.nos ?? {});
    setZoom(feed.layout.zoom ?? 1);
    setPan({ x: feed.layout.panX ?? 0, y: feed.layout.panY ?? 0 });
  }, [feed?.layout, sujo]);
  ...
  const salvarMapa = async () => {
    setSalvando(true);
    const { error } = await supabase.functions.invoke("warroom-feed", {
      body: { salvarLayout: { nos: posManual, zoom, panX: pan.x, panY: pan.y } },
    });
    setSalvando(false);
    if (!error) { setSujo(false); setEditando(false); }
  };
      ...
      <div className="wr-tv">
        ...
        <footer className="wr-base">
          ...
          <div className="wr-numero">
            ...
            <span className="wr-nota">{feed?.numeros.cacheTaxa ?? 0}% servidos por cache</span>
          </div>
          <div className="wr-numero">
