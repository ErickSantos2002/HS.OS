...
⚠️ **Não mova o contexto para a raiz do repositório.** Tentado em 17/08/2026
para levar a pasta `skills/` junto, e o deploy quebrou na hora: o EasyPanel
monta o comando com o caminho fixo `code/backend/`, então `COPY backend/…`
falha com `"/backend": not found`. O conserto foi o inverso — a pasta passou a
morar em `backend/skills/`, dentro do contexto.
